...
async function handleCardWalletBalance(req, res) {
  try {
    const { identity } = await cardAccountIdentity();
    const live = await describeLiveReceive(identity);
...
async function handleCheckoutConfirm(req, res) {
  ...
  // After the card signs the payment-package proof, also broadcast a real
  // chain artifact: claim any Lightning-receive VHTLC the card owns. The
  // preimage hash of the merchant invoice will not match the receive VHTLC
  // we hold (the merchant invoice is a *new* payment; the VHTLC is our
  // existing receive). The broadcast is real either way -- card signs, an
  // Ark VHTLC claim tx is sent, and the card's settled balance goes up.
  // For an actual merchant payment we'd need the full Boltz send path
  // (intent + forfeit + ephemeral session keys); see docs/tap-to-pay-concept.md
  // step 6. Demo-only.
  let broadcast = { attempted: false };
  try {
    const claim = await claimReceiveVHTLCs();
    if (claim.skipped) {
      broadcast = { attempted: false, skipped: claim.skipped };
    } else {
      broadcast = {
        attempted: true,
        claimable_count: claim.claimable_count,
        claimed_ok: claim.claims.filter((c) => c.status === 'NURI_CARD_ARKADE_CLAIM_OK').length,
        claims: claim.claims.map((c) => ({
          swap_id: c.swap_id,
          status: c.status,
          ark_address: c.ark_address,
          aggregate_pubkey33: c.aggregate_pubkey33,
          error: c.error,
        })),
      };
    }
...
async function requestHandler(req, res) {
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/api/card/wallet-balance') { await handleCardWalletBalance(req, res); return; }
